...
class ContentContract(object):
    ...
    def __init__(self, keystore_file=None, keystore_pwd=None,
        ...
        self.gas_limit = Config.BLOCK_GAS_LIMIT
    ...
    def _load_wallet(wallet_file):
        if not os.path.isfile(wallet_file):
            wallet_file = os.path.join(Config.CURR_DIR, 'resources', 'keystore', wallet_file)
            if not os.path.isfile(wallet_file):
    ...
    def _save_wallet(wallet, file_name):
        wf = os.path.join(Config.CURR_DIR, 'resources', 'keystore', "{}.json".format(file_name))
    ...
    def _load_abi(self):
        abi_path = os.path.join(Config.CURR_DIR, 'abi')
    ...
    def _load_contract(self):
        # 读取合约地址
        with open(os.path.join(Config.CURR_DIR, "contractAddresses.json")) as wf:
    ...
    def func_call(self, contract_name, function, param):
        ...
        if function in self.contract[contract_name].view_funcs:
            addr=self.valid_address(Config.main_address)
...
class ReceiptWatcher(ContentContract):
    ...
    def run(self, *args):
        while True:
            try:
                time.sleep(Config.receipt_watcher_sleep_time)
    ...
    def send_newbie_token(self, address):
        receipt = self.func_call("UshareToken", "transfer", [address, Config.newbie_token])
        return receipt
